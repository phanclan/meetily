...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'meetnola-summary-eval-'));
...
try {
  ...
  const run = spawnSync('cargo', ['test', '--features', 'meetnola', '--lib', 'summary::quality_evals::live_summary_quality', '--', '--exact', '--ignored', '--nocapture'], {
    cwd: path.join(frontend, 'src-tauri'),
    stdio: 'inherit',
    env: {
      ...process.env,
      CARGO_TARGET_DIR: process.env.CARGO_TARGET_DIR || path.resolve(frontend, '../target/meetnola'),
      MEETNOLA_EVAL_CONTEXT: process.env.MEETNOLA_EVAL_CONTEXT || 'runtime',
      MEETNOLA_SUMMARY_EVAL_CONTEXTS: input,
    },
  });
