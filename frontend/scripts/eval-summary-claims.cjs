...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'meetnola-claim-eval-'));
...
try {
  ...
  const run = spawnSync('cargo', ['test', '--features', 'meetnola', '--lib', 'live_summary_claim_checks', '--', '--ignored', '--nocapture'], {
    cwd: path.join(frontend, 'src-tauri'),
    stdio: 'inherit',
    env: {
      ...process.env,
      CARGO_TARGET_DIR: process.env.CARGO_TARGET_DIR || path.resolve(frontend, '../target/meetnola'),
      MEETNOLA_CLAIM_EVAL_INPUT: input,
    },
  });
