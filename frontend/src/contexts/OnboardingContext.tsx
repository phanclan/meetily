...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const unlisten = listen<{
      ...
      (event) => {
        ...
        // Optional local summary downloads can still update onboarding state if the user starts one.
  ...
  const verifyModelStatus = async (savedStatus: OnboardingStatus) => {
    ...
    // Verify if any optional local summary model exists on disk.
    try {
  ...
  // Local model downloads are optional and no longer start during onboarding.
  const startBackgroundDownloads = async (_includeGemma: boolean) => {
    console.log('[OnboardingContext] Local model downloads are opt-in and skipped during onboarding');
