...
export function SummaryLanguageSettings() {
  ...
  return (
    <div className="border-b border-stone-200 py-5 relative">
      <div className="flex items-center gap-2 mb-2">
        <Globe size={18} className="text-stone-500" />
        <h3 className="text-base font-semibold text-stone-900">Summary Language</h3>
      </div>
            ...
            <span
              ...
              className={`inline-flex items-center rounded-full border text-sm overflow-hidden ${
                isPinned
                  ...
                  : 'bg-gray-100 border-stone-200 text-stone-800'
              }`}
            >
              <button
                type="button"
                ...
                className={`flex items-center gap-1.5 pl-3 pr-2 py-1 hover:brightness-95 active:brightness-90 ${
                  isPinned ? 'text-blue-800' : 'text-stone-800'
                }`}
              >
                <Pin
                  size={14}
                  className={isPinned ? 'text-blue-600' : 'text-stone-400'}
              ...
              <button
                ...
                className={`pr-2.5 pl-0.5 py-1 leading-none ${isPinned ? 'text-blue-400 hover:text-blue-700' : 'text-stone-400 hover:text-stone-700'}`}
              >
                ×
              </button>
            </span>
        ...
        <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
          <PopoverTrigger asChild>
            <button
              ...
              className="inline-flex items-center gap-1 rounded-full border border-dashed border-gray-300 px-3 py-1 text-sm text-stone-600 hover:border-gray-400 hover:text-stone-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              ＋ Add language
            </button>
          </PopoverTrigger>
