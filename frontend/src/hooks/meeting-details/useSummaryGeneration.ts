...
export function useSummaryGeneration({
  ...
  }, [meeting.id, modelConfig, setAiSummary, updateMeetingTitle, onMeetingUpdated, onOpenModelSettings]);
  ...
  useEffect(() => {
    if (generationBusy.current || !['pending', 'processing', 'summarizing', 'regenerating'].includes(initialSummaryStatus?.toLowerCase() || '')) return;
    const request = { ready: Promise.resolve(), started: true };
    generationStart.current = request;
    generationBusy.current = true;
    setSummaryStatus('processing');
    observeGeneration(request, true);
  }, [meeting.id, initialSummaryStatus, observeGeneration]);
  ...
  const processSummary = useCallback(async ({
    ...
    try {
      ...
      if (generationStart.current === request) observeGeneration(request, isRegeneration, process_id);
    } catch (error) {
  ...
  const handleGenerateSummary = useCallback(async (customPrompt: string = '') => {
    ...
    if (generationEpoch.current === epoch) await processSummary(buildSummarySourcePayload(allTranscripts, customPrompt));
  ...
  // Lock before the first transcript/model read, across both entry points.
  // Accepted jobs retain the lock until polling confirms a terminal result.
  const runGeneration = useCallback(async (start: () => Promise<void>) => {
    if (generationBusy.current) return;
    generationBusy.current = true;
    generationStart.current = null;
    const epoch = generationEpoch.current;
    const wasAccepted = () => generationStart.current?.started === true;
    try {
      await start();
    } finally {
      if (generationEpoch.current === epoch && !wasAccepted()) generationBusy.current = false;
    }
  }, []);
  const generateSummary = useCallback((customPrompt = '') =>
    runGeneration(() => handleGenerateSummary(customPrompt)), [runGeneration, handleGenerateSummary]);
  const regenerateSummary = useCallback(() =>
    runGeneration(handleRegenerateSummary), [runGeneration, handleRegenerateSummary]);

  return {
    summaryStatus,
    summaryError,
    handleGenerateSummary: generateSummary,
    handleRegenerateSummary: regenerateSummary,
