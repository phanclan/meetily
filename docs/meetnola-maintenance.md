...
Enhancement currently uses Ollama with `qwen3.5:4b-mlx` ([official model](https://ollama.com/library/qwen3.5:4b-mlx)). Meetnola requests `reasoning_effort: "none"` for the canonical Ollama `qwen3.5` and `qwen3.6` model families so meeting summaries and questions do not wait for extended reasoning. Other providers and models retain their existing request parameters. The smaller model favors latency; generated summaries still require review for unsupported detail, particularly with sparse source notes.
