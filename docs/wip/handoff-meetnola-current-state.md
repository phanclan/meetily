...
Active dev bundle: `/private/tmp/meetnola Dev.app`, build `20260908-2118-notes-workspace`, with frontend hot reload on port 3118. The source-coverage fixes and updated standard template are installed and visually verified. Local Parakeet Compact and Ollama `qwen3.5:4b-mlx` remain selected; recording is stopped. The standalone tester remains unchanged.

The saved-note toolbar is compact, with Copy in Meeting actions and accessible model/template controls. Generated sections use semantic H2 headings styled below the document title. Save is enabled only for edited enhanced notes; status distinguishes unsaved, saving, failed, and saved. The editor awaits persistence and retains dirty state after a failed save or further edits during a save. Regenerated Markdown loads without falsely marking the document dirty. Switching between original and enhanced notes preserves enhanced edits; the document Home button saves pending enhanced edits before navigation. Other navigation and app-close paths have not been extended in this batch.
...
Validation: 44 frontend workflow tests, three Markdown tests, TypeScript, production frontend build, 88 native summary tests (one live-model test ignored), one native title-persistence test, native dev build, signature verification, and diff checks passed. Native UI checks used only the synthetic **Meetnola saved-note review**: checkbox save/reopen, retention across note tabs, Home save/reopen, regeneration without a false dirty warning, title preservation after regeneration/reopen, model/template controls and Escape focus, and a half-screen layout check. Regeneration still invented an implementation commitment from sparse source text; factual quality is not accepted.
