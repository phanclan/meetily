...
Pending runtime verification: follow-up chat and Gemma sampling changes are implemented on `main`. Chat sends the last six successful exchanges separately from the current meeting sources, removes old citation IDs from historical answers, and clears history with the conversation or meeting. Seventy frontend workflow tests, TypeScript, 92 native summary tests, an isolated production frontend build, and native build `20260908-2212-gemma-follow-ups` passed. That binary is not installed. A restart request made while the user was in the model picker remains unanswered. A later read-only UI check confirmed Ollama `qwen3.5:4b-mlx` is still selected. Finish native installation, Gemma selection, and synthetic enhancement/follow-up UI checks when restart is authorized. The optional `live_meeting_follow_up_quality` test passed against local Gemma: a follow-up retained the title-preservation subject and source citation, and an invented prior assignment was rejected against the original notes. The app process was confirmed still running; no restart or selection change was performed.

Gemma recommendation: the user's requested small MLX candidate is `gemma4:e4b-mlx`, downloaded through Ollama. Google calls it E4B (8B total with embeddings, 4.5B effective); the installed artifact reports 8.1B, Safetensors, NVFP4, and Ollama shows 100% GPU execution. See the [Google model card](https://huggingface.co/google/gemma-4-E4B-it) and [official Ollama MLX tag](https://ollama.com/library/gemma4:e4b-mlx). Two default-temperature trials preserved all tested task assignments, but manual review of the repeat caught a validation condition attached to the wrong event. At temperature 0.2, all nine synthetic cases passed and manual review found the relevant facts intact: 2.58–4.58 seconds per case, median 3.23 seconds. The current Qwen baseline passed six of nine checks, median 1.08 seconds. These are short synthetic cases, not long-meeting performance claims. The new Gemma profile applies temperature 0.2 to Ollama Gemma 4 requests; other providers retain their settings. A narrow reassignment phrase check was expanded to accept "moved from Noah to Liam," with regression coverage still rejecting the old owner/deadline. Gemma is recommended but not selected yet.

## Saved-note workspace and reliable edits

Active dev bundle: `/private/tmp/meetnola Dev.app`, build `20260908-2155-cited-answers`, with frontend hot reload on port 3118. The source-coverage fixes and updated standard template are installed and visually verified. Local Parakeet Compact and Ollama `qwen3.5:4b-mlx` remain selected; recording is stopped. The standalone tester remains unchanged.
