...
- The earlier quality pass used dev badge `20260905-1244-4957e34` with frontend code through `3aee9b4`. The newer running dev build is recorded above. This session uses a standalone Next dev server, so Rust changes require rebuilding and relaunching the native app. The packaged release bundle does not include the subsequent design fixes.
