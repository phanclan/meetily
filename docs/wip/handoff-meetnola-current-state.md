...
Commit checkpoint (September 9): the accumulated framework upgrade, persistent conversations/search, note-workspace fixes, and summary-quality work are being committed in three local commits at the user's request. Earlier uncommitted-status statements below describe their original validation passes. Continue from the committed baseline on main; the next investigation is decision placement, and no push is authorized. The final pre-commit refresh passed TypeScript, 102 workflow tests, three Markdown tests, and eight native Meetnola regressions (one live-model test ignored); native summary/build and visual evidence remain recorded below.
